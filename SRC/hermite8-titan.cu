...
template<>
__device__ void reduce_final<1, 12>(const double x, double *dst){
		...
		Gravity::GForce        *ftot)
{
	...
	reduce_final<Gravity::NJREDUCE/32, 12> (y, ddst);
}
		...
		Force        force[] )
{
	...
	for(int i=0; i<ni; i++){
		...
		force[is+i].snp = f.snp;
		force[is+i].crk = f.crk;
	}
}
		...
		Force        force[] )
{
	...
	for(int ii=0; ii<nact; ii+=NIMAX){
		...
		for(int i=0; i<nistore; i++){
			...
			force[istore+i].snp = f.snp;
			force[istore+i].crk = f.crk;
		}
		{   // reduction
			const int nword = sizeof(GForce) / sizeof(double);
			assert(12 == nword);
