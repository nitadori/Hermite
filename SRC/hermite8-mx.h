...
struct Gravity{
	...
	struct GForce{
		...
		GForce(	
				...
				const v4r8 _cx, const v4r8 _cy, const v4r8 _cz)
...
#pragma omp parallel
			...
			GParticle      * __restrict ptcl)
	{
		...
		v4r8 ym5  = v4r8::load(&p->d4a.x);
		v4r8 ym6  = v4r8::load(&p->d5a.y);
		...
		ym4 .store(ptr + 16);
		ym5 .store(ptr + 20);
		const v4_mask v2mask(1,1,0,0);
		ym6 .store(ptr + 24, v2mask);
		...
		ptcl[addr].d4a[0] = p->d4a.x;
		ptcl[addr].d4a[1] = p->d4a.y;
		ptcl[addr].d4a[2] = p->d4a.z;
		ptcl[addr].d5a[0] = p->d5a.x;
		ptcl[addr].d5a[1] = p->d5a.y;
		ptcl[addr].d5a[2] = p->d5a.z;
#endif
	}
