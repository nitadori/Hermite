#pragma once
#include<cstdlib>
#include<cassert>

#include <omp.h>

#include "v2r8.h"
#include "v4r8.h"

struct Gravity{
	enum{
		NIMAX = 1024,
		MAXTHREAD = 16,
	};

	struct GParticle{
		v2r8 mass;
		v2r8 tlast;
		v2r8 posH[3];
		v2r8 posL[3];
		v2r8 vel[3];
		v2r8 acc[3];
		v2r8 jrk[3];
		v2r8 snp[3];
		v2r8 crk[3];
		v2r8 d4a[3];
		v2r8 d5a[3]; // 29 XMM
	};

	struct GPredictor{
		v2r8 mass;
		v2r8 posH[3];
		v2r8 posL[3];
		v2r8 vel[3];
		v2r8 acc[3];
		v2r8 jrk[3];
	};

	struct GForce{
		v2r8 ax, ay, az;
		v2r8 jx, jy, jz;
		v2r8 sx, sy, sz;
		v2r8 cx, cy, cz;

		void clear(){
			ax = ay = az = v2r8(0.0);
			jx = jy = jz = v2r8(0.0);
			sx = sy = sz = v2r8(0.0);
			cx = cy = cz = v2r8(0.0);
		}

		void save(
				const v2r8 _ax, const v2r8 _ay, const v2r8 _az,
				const v2r8 _jx, const v2r8 _jy, const v2r8 _jz,
				const v2r8 _sx, const v2r8 _sy, const v2r8 _sz,
				const v2r8 _cx, const v2r8 _cy, const v2r8 _cz)
		{
			ax = _ax; ay = _ay; az = _az;
			jx = _jx; jy = _jy; jz = _jz;
			sx = _sx; sy = _sy; sz = _sz;
			cx = _cx; cy = _cy; cz = _cz;
		}
	};

	const int  nbody;
	GParticle  *ptcl;
	GPredictor *pred;

	Gravity(const int _nbody) : nbody(_nbody) {
		ptcl = allocate<GParticle,  128> (nbody);
		pred = allocate<GPredictor, 128> (nbody);
#pragma omp parallel
		assert(MAXTHREAD >= omp_get_thread_num());
	}
	~Gravity(){
		free(ptcl);
		free(pred);
	}

	static void set_jp_rp(
			const int addr,
			const Particle * __restrict p,
			GParticle      * __restrict ptcl)
	{
		const unsigned ah = unsigned(addr)/2;
		const unsigned al = unsigned(addr)%2;

		const double mass  = p->mass;
		const double tlast = p->tlast;
		const dvec3  posH  = p->posH;
		const dvec3  posL  = p->posL;
		const dvec3  vel   = p->vel;
		const dvec3  acc   = p->acc;
		const dvec3  jrk   = p->jrk;
		const dvec3  snp   = p->snp;
		const dvec3  crk   = p->crk;
		const dvec3  d4a   = p->d4a;
		const dvec3  d5a   = p->d5a;

		double *dst = (double *)(ptcl + ah) + al;
		dst[ 0] = mass;
		dst[ 2] = tlast;
		dst[ 4] = posH.x;
		dst[ 6] = posH.y;
		dst[ 8] = posH.z;
		dst[10] = posL.x;
		dst[12] = posL.y;
		dst[14] = posL.z;
		dst[16] = vel.x;
		dst[18] = vel.y;
		dst[20] = vel.z;
		dst[22] = acc.x;
		dst[24] = acc.y;
		dst[26] = acc.z;
		dst[28] = jrk.x;
		dst[30] = jrk.y;
		dst[32] = jrk.z;
		dst[34] = snp.x;
		dst[36] = snp.y;
		dst[38] = snp.z;
		dst[40] = crk.x;
		dst[42] = crk.y;
		dst[44] = crk.z;
		dst[46] = d4a.x;
		dst[48] = d4a.y;
		dst[50] = d4a.z;
		dst[52] = d5a.x;
		dst[54] = d5a.y;
		dst[56] = d5a.z;
	}
	void set_jp(const int addr, const Particle &p){
		set_jp_rp(addr, &p, ptcl);
	}

	static void predict_all_rp(
			const int nbody, 
			const double tsys, 
			const GParticle * __restrict ptcl,
			GPredictor      * __restrict pred);
	void predict_all(const double tsys){
		predict_all_rp(nbody, tsys, ptcl, pred);
	}

	void calc_force_in_range(
			const int    is,
			const int    ie,
			const double deps2,
			Force        force[] );
	void calc_force_on_first_nact(
			const int    nact,
			const double eps2,
			Force        force[] )
	{
		for(int ii=0; ii<nact; ii+=NIMAX){
			const int ni = (nact-ii) < NIMAX ? (nact-ii) : NIMAX;
			calc_force_in_range(ii, ii+ni, eps2, force);
		}
	}

	static void calc_potential_rp(
			const int    nbody,
			const double deps2,
			const GParticle * __restrict ptcl,
			v2r8           (* __restrict xmbuf)[4],
			double          * __restrict potbuf);
	void calc_potential(
			const double deps2,
			double * __restrict potbuf)
	{
		v2r8 (*xmbuf)[4] = allocate<v2r8[4], 128>(nbody+1);
		calc_potential_rp(nbody, deps2, ptcl, xmbuf, potbuf);
		free(xmbuf);
	}
};
